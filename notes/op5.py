    ...
    def _add_constraints(self):
        ...
        for j, pallet in enumerate(self.pallets):
            ...
            for i in assembled_items:
                z_i = self.item_positions[(i, j)][2]
                h_i = self.items[i]['height']

                for k in assembled_items:
                    if i != k:
                        z_k = self.item_positions[(k, j)][2]
                        h_k = self.items[k]['height']
                        # Ensure assembled items are not stacked on top of each other
                        self.model.Add(z_i + h_i <= z_k).OnlyEnforceIf(self.item_in_pallet[(i, j)]).OnlyEnforceIf(self.item_in_pallet[(k, j)])
                        self.model.Add(z_k + h_k <= z_i).OnlyEnforceIf(self.item_in_pallet[(i, j)]).OnlyEnforceIf(self.item_in_pallet[(k, j)])
...
items = [
    ...
    {'weight': 15, 'length': 3, 'width': 3, 'height': 1, 'assembled': True, 'bundle': False},
